...
#define CHECK(fml, name, printer, msg) \
  ...
  for (auto &src_instr : t.src.instrs()) {
    if (const Annotate *src_annotate = dynamic_cast<const Annotate*>(&src_instr)) {
      ...
      if (!config::disallow_ub_exploitation) {
        if (!check(instr_dom && !poison_cnstr, "annot_poison", print_value, "Target is more poisonous than source")) {
          return;
        }
        annotation_axioms.add(instr_dom.implies(poison_cnstr));
      }

      if (!check(instr_dom && !value_cnstr, "annot_value", print_value, "Value mismatch")) {
        return;
      }
